...
static PyObject *sipEnumType_get_qualname(sipEnumTypeObject *eto,
        void *closure);
 ...
 */
static PyGetSetDef sipEnumType_getset[] = {
    {(char *)"__qualname__", (getter)sipEnumType_get_qualname, NULL, NULL,
            NULL},
        ...
        PyObject *mod_dict)
{
     ...
     * so we remove it.  We will create the correct one in the getter if it is
     * ever called.
     */
     if (etd->etd_scope >= 0)
        Py_CLEAR(((PyHeapTypeObject *)py_type)->ht_qualname);
#endif
...
/*
 * The __qualname__ getter for an enum.
 */
static PyObject *sipEnumType_get_qualname(sipEnumTypeObject *eto,
        void *closure)
{
    ...
    if (eto->super.ht_qualname == NULL)
    {
        sipEnumTypeDef *etd;
        sipTypeDef *scope_td;
        PyObject *scope_type, *scope_qualname;

        /* Get the type that is the enum's scope. */
        etd = (sipEnumTypeDef *)eto->type;
        scope_td = eto->type->td_module->em_types[etd->etd_scope];
        scope_type = (PyObject *)sipTypeAsPyTypeObject(scope_td);

        /* Get the scope's __qualname__. */
        scope_qualname = PyObject_GetAttrString(scope_type, "__qualname__");

        if (scope_qualname == NULL)
            return NULL;

        /* Append the name of the enum. */
        eto->super.ht_qualname = PyUnicode_FromFormat("%U.%s", scope_qualname,
                sipPyNameOfEnum(etd));
    ...
    Py_INCREF(eto->super.ht_qualname);
    return eto->super.ht_qualname;
}
